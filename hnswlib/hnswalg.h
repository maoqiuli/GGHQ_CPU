...
namespace hnswlib {
    ...
    public:
        ...
        void Distribution_Connectivity()
        {
            int node_num = 0;
            int block = 10;
            int node_dis[101] = {0};
            float *p = new float[max_elements_];
            for (int cur_node_id = 0; cur_node_id < max_elements_; cur_node_id++)
            {
                int *data = (int *) get_linklist0(cur_node_id);
                size_t size = getListCount((linklistsizeint*)data);
                int same_attr_size = 0;
                node_num += 1;
                unsigned cur_bucket_id = *(getBucketIdByInternalId(cur_node_id));
                for (int j = 1; j <= size; ++j)
                {
                    tableint nei_id = *(data + j);
                    unsigned nei_bucket_id = *(getBucketIdByInternalId(nei_id));
                    if (nei_bucket_id == cur_bucket_id) same_attr_size++;
                }
                *(p + cur_node_id) = ((float)same_attr_size) / size;
                int ind = block * same_attr_size / size;
                node_dis[ind] += 1;
            }
            
            std::cout << "node number : " << node_num << std::endl
                        << "node same attribute distribution : " << "\t";
            for (int i = 0; i < block; ++i) 
            {
                if (i != block - 1)
                    std::cout << node_dis[i] << "  ";
                else
                    std::cout << node_dis[i] + node_dis[i+1] << "  ";
            }
            std::cout << std::endl;
            float mean = 0.0;
            float var = 0.0;
            ...
            for (int cur_node_id = 0; cur_node_id < max_elements_; cur_node_id++)
            {
                var += (*(p + cur_node_id) - mean) * (*(p + cur_node_id) - mean);
            }
            var /= max_elements_;
...
//                    if (candidate_id == 0) continue;
        ...
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
        searchBaseLayerST(vector<tableint> ep_id, const void *data_point, size_t ef, std::vector<bool> bucket_id_table, double *path_distance) const {
            VisitedList *vl = visited_list_pool_->getFreeVisitedList();
            vl_type *visited_array = vl->mass;
            vl_type visited_array_tag = vl->curV;
            ...
            while (!candidate_set.empty()) {
                ...
                if (path_id < PATH_LEN)
                    path_distance[path_id++] += -current_node_pair.first;
#endif
        ...
        template <bool has_deletions, bool collect_metrics=false>
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
        searchBaseLayerSTEc(vector<tableint> ep_id, const void *data_point, size_t ef, std::vector<bool> bucket_id_table, double *path_distance) const {
            VisitedList *vl = visited_list_pool_->getFreeVisitedList();
            vl_type *visited_array = vl->mass;
            vl_type visited_array_tag = vl->curV;
            ...
            dist_t lowerBound;
            // dist_t dist_last = std::numeric_limits<dist_t>::max();
            lowerBound = std::numeric_limits<dist_t>::max();
            ...
            while (!candidate_set.empty()) {
                ...
                // dist_t dist_now = -current_node_pair.first;
                ...
                for (size_t j = 1; j <= size; j++) {
                    ...
                    if ((candidate_is_target == false && current_node_is_target ==false)) {
                        continue;
                    }
        ...
        std::priority_queue<std::pair<dist_t, labeltype >>
        searchKnn(const void *query_data, size_t k, std::vector<bool> bucket_id_table, const unsigned enterpoint_num, const bool strategy, double *path_distance) const {
            std::priority_queue<std::pair<dist_t, labeltype >> result;
            ...
            if (num_deleted_) {
                if (strategy) {
                    top_candidates=searchBaseLayerSTEc<true,true>(
                        currObj, query_data, std::max(ef_, k), bucket_id_table, path_distance);
                }
                else {
